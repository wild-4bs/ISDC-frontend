  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-dashboard-bg overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] rounded-lg border p-0 shadow-lg duration-200 sm:max-w-lg",
          className,
        )}
        {...props}
      >
...
function DialogHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-2 text-center sm:text-start bg-dashboard-bg p-6 pb-2",
        className,
      )}
      {...props}
    />
...
function DialogBody({
  ...
  return (
    <div
      {...props}
      className={cn("bg-background px-6 py-4 m-2 rounded-lg", className)}
    >
      {children}
    </div>
  );
